...
class MT5Manager:
    ...
    # ─── Symbol Info & Market Data ────────────────────────────────────

    def get_symbol_info(self, symbol: str) -> dict:
        """
        Return current market data for a symbol.
        Used by the autonomous agent's market data tool.
        """
        if not self.ensure_connected():
        ...
        with self._lock:
            try:
                ...
                if tick is None or info is None:
                    ...
                    info = mt5.symbol_info(symbol)
                    if tick is None or info is None:
                        return {"error": f"Symbol {symbol} not found in MT5 terminal"}

                pip_mul = 100.0 if "JPY" in symbol.upper() else 10000.0
                spread_pips = round((tick.ask - tick.bid) * pip_mul, 1)
    ...
    def place_market_order(self, symbol: str, direction: str, volume: float,
        ...
        """
        Convenience wrapper — delegates to MT5TradeExecutor.
        Called by the autonomous agent execution layer.
        """
        try:
            from backend.utils.mt5_trade_executor import mt5_trade_executor
            return mt5_trade_executor.place_order(
                symbol=symbol,
                direction=direction,
                volume=volume,
                sl=sl,
            ...
            return {"success": False, "error": f"mt5_trade_executor not available: {e}"}
        except Exception as e:
    ...
    def close_position(self, ticket: int) -> dict:
        ...
        try:
            from backend.utils.mt5_trade_executor import mt5_trade_executor
            return mt5_trade_executor.close_position(ticket)
        except Exception as e:
            return {"success": False, "error": str(e)}
    ...
    def close_all_positions(self) -> list:
        ...
        try:
            from backend.utils.mt5_trade_executor import mt5_trade_executor
            return mt5_trade_executor.close_all_positions()
        except Exception as e:
            return [{"success": False, "error": str(e)}]
